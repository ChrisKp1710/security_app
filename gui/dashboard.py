...
class Dashboard(ctk.CTk):
    ...
    def setup_home_ui(self):
        ...
        ctk.CTkLabel(card2, text="Security Engine", font=("Roboto", 12), text_color="gray").pack(pady=(20, 5))
        self.lbl_stat_pwd = ctk.CTkLabel(card2, text="Active", font=("Roboto", 24, "bold"), text_color=COLOR_SUCCESS)
    ...
    def setup_network_ui(self):
        ...
        self.btn_clear = ctk.CTkButton(self.network_frame, text="CLEAR", height=30, width=80, fg_color="transparent", 
                                       border_width=1, text_color=COLOR_DANGER, border_color=COLOR_DANGER, command=self.clear_console)
    ...
    def clear_console(self):
        ...
        self.show_toast("Console cleared", COLOR_INFO)
    ...
    def mostra_risultati_scan(self, risultati):
        ...
        self.show_toast("Scan analysis completed")

    def avvia_dir(self):
        t = self.entry_ip.get().strip()
        self.check_and_clear_logs()
        self.btn_dir.configure(state="disabled")
        self.log(f"Starting directory discovery on {t}...", "INFO")
    ...
    def mostra_dir(self, res):
        ...
        if res is None: self.log("Connection Error: Target unreachable.", "DANGER")
        elif not res: self.log("No hidden resources identified.", "WARNING")
        else:
            self.log(f"Identified {len(res)} resources:", "SUCCESS")
            self.report_data["directories"] = res
            for r in res: self.log(f"  📂 {r}", "SUCCESS")
        self.show_toast("Directory search finished")
    ...
    def gestisci_hash(self):
        ...
        if f:
            ...
            self.show_toast("File hash calculated")

    def confirm_export(self):
        formats = []
        if self.var_html.get(): formats.append("html")
        ...
        if folder:
            ...
            generate_reports(self.report_data, formats, folder)
            self.hide_export_overlay()
            self.show_toast("Audit reports saved successfully")
